...
const required = ["pnpm-lock.yaml", "packages/desktop/src-tauri/Cargo.lock"];
...
if (
  ...
  throw new Error(
    "Lockfile bootstrap may only push a production rebuild branch; got " +
      (branch ?? "unknown"),
  );
}

execFileSync("git", ["config", "user.name", "ORBIT Automation"], {
  stdio: "inherit",
});
execFileSync(
  "git",
...
if (stagedDiff.status !== 1) {
  throw new Error(
    "git diff --cached --quiet failed with status " + stagedDiff.status,
  );
}

execFileSync(
  "git",
  ["commit", "-m", "chore(ci): bootstrap reproducible lockfiles"],
  {
    stdio: "inherit",
  },
);
