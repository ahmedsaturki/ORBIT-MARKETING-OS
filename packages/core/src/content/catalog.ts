...
export function validateContentItem(
  ...
  if (!content.workspaceId.trim())
    errors.push("content workspace id is required");
  ...
  if (content.tags.some((tag) => !tag.trim()))
    errors.push("content tags must be non-empty");
  if (
    new Set(content.tags.map((tag) => tag.trim().toLowerCase())).size !==
    content.tags.length
  ) {
    errors.push("content tags must be unique");
  }
  if (
    Number.isNaN(Date.parse(content.createdAt)) ||
    Number.isNaN(Date.parse(content.updatedAt))
  ) {
...
export function validateContentForDelivery(
  ...
  if (content.approvalStatus !== "approved")
    errors.push("content is not approved");
  if (!selectContentVariant(content, platform))
    errors.push("content variant is empty");
  return { valid: errors.length === 0, errors };
}
