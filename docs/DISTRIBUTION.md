...
`release-desktop.yml` builds Windows, Linux, and macOS bundles when a `v*` tag is pushed.

The current pipeline deliberately produces **unsigned** desktop artifacts. It also generates `SHA256SUMS.txt` so users can verify downloaded files.

Signing is a separate release gate and must never use private signing keys committed to the repository.

## Mobile

The Expo project supports local development and web export. Native Android/iOS distribution remains a separate gate because production signing, store accounts, and native build environments are product-release concerns.
