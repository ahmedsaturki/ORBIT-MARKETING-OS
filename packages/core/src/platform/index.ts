...
function validateCapabilities(capabilities: readonly PlatformCapability[], errors: string[]): void {
  ...
  for (const capability of capabilities) {
    ...
    if (capability.requiredScopes.some((scope) => !scope.trim())) errors.push("empty_capability_scope");
    if (capability.externallyVisible && capability.requiredScopes.length === 0) {
      errors.push("external_capability_scope_required");
    }
  }
}

export function validatePlatformExtensionManifest(manifest: PlatformExtensionManifest): PlatformValidationResult {
  const errors: string[] = [];
  ...
  if (!VERSION_PATTERN.test(manifest.version)) errors.push("extension_version_invalid");
  if (!VERSION_PATTERN.test(manifest.minOrbitVersion)) errors.push("min_orbit_version_invalid");
  if (manifest.capabilities.length === 0) errors.push("extension_capability_required");
  if (manifest.requiredPermissions.some((permission) => !permission.trim())) errors.push("empty_required_permission");
  validateCapabilities(manifest.capabilities, errors);
  return { valid: errors.length === 0, errors };
}

export function validateConnectorManifest(manifest: ConnectorExtensionManifest): PlatformValidationResult {
  ...
  if (manifest.authModes.length === 0) errors.push("connector_auth_mode_required");
  if (manifest.authModes.includes("user_authorized_browser") && !manifest.requiredPermissions.includes("browser:authorized-session")) {
    errors.push("browser_session_permission_required");
  }
  return { valid: errors.length === 0, errors };
}

export function validateVerticalPackManifest(manifest: VerticalPackManifest): PlatformValidationResult {
  const errors = [...validatePlatformExtensionManifest(manifest).errors];
  requiredText(manifest.vertical, "vertical_required", errors);
  if (manifest.lifecycleStages.length < 2) errors.push("lifecycle_stage_required");
  if (manifest.objectTypes.length === 0) errors.push("object_type_required");
  if (manifest.workflowIds.some((id) => !id.trim())) errors.push("empty_workflow_id");
  return { valid: errors.length === 0, errors };
}
...
export class PlatformRegistry {
  ...
  public register(extension: PlatformExtensionManifest): void {
    ...
    if (!validation.valid) throw new Error("invalid_extension:" + validation.errors.join(","));
    if (this.extensions.has(extension.id)) throw new Error("extension_duplicate");
  ...
  public list(kind?: PlatformExtensionKind): readonly PlatformExtensionManifest[] {
...
function cloneManifest(manifest: PlatformExtensionManifest): PlatformExtensionManifest {
  return {
    ...manifest,
    capabilities: manifest.capabilities.map((capability) => ({ ...capability, requiredScopes: [...capability.requiredScopes] })),
    requiredPermissions: [...manifest.requiredPermissions],
  };
}

export function isConnectorManifest(manifest: PlatformExtensionManifest): manifest is ConnectorExtensionManifest {
  return manifest.kind === "connector";
}

export function isVerticalPackManifest(manifest: PlatformExtensionManifest): manifest is VerticalPackManifest {
