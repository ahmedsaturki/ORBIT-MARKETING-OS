...
export function validateCampaign(campaign: Campaign): CampaignValidationResult {
  ...
  if (campaign.accountIds.length === 0) errors.push("at least one account is required");
 ...
 */
export function validateTask(task: Task, campaign: Campaign): CampaignValidationResult {
  const errors: string[] = [];

  if (task.campaignId !== campaign.id) errors.push("task campaign mismatch");
  if (!campaign.accountIds.includes(task.accountId)) errors.push("account is not part of campaign");
