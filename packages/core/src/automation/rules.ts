...
export function validateAutomationRulePack(
  ...
  if (pack.schemaVersion !== 1)
    errors.push("unsupported rule pack schema version");
  ...
  for (const rule of pack.rules) {
    ...
    if (rule.platform !== pack.platform)
      errors.push("rule platform must match rule pack platform");
    if (rule.taskKinds.some((kind) => !TASK_KINDS.includes(kind))) {
      errors.push("rule contains unsupported task kind");
    }
    if (rule.taskKinds.length === 0)
      errors.push("rule must target at least one task kind");
    if (
      rule.enabled &&
      rule.taskKinds.some((kind) => kind !== "sync") &&
    ...
    if (
      !Number.isInteger(rule.timeoutMs) ||
      rule.timeoutMs < 1000 ||
      rule.timeoutMs > 300000
    ) {
