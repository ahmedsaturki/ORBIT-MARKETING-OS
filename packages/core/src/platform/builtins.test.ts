...
import { BUILT_IN_CONNECTOR_MANIFESTS, BUILT_IN_PLATFORM_MANIFESTS, BUILT_IN_VERTICAL_PACKS } from "./builtins.js";
import { PlatformRegistry, validateConnectorManifest, validateVerticalPackManifest } from "./index.js";

describe("platform built-ins", () => {
  it("keeps current real connector capability contracts explicit", () => {
    expect(BUILT_IN_CONNECTOR_MANIFESTS.map((item) => item.platform)).toEqual(["telegram", "linkedin"]);
