...
export interface ApprovalDecision {
  ...
  readonly reason: "not_required" | "pending" | "approved" | "rejected" | "changes_requested";
}

/**
 * Evaluates whether a content item has cleared its configured approval gate.
 */
export function evaluateApproval(content: ContentItem, approval?: Approval): ApprovalDecision {
  ...
  switch (approval.status) {
    case "approved":
      return { status: "approved", allowedToPublish: true, reason: "approved" };
    case "rejected":
      return { status: "rejected", allowedToPublish: false, reason: "rejected" };
    case "changes_requested":
      return { status: "changes_requested", allowedToPublish: false, reason: "changes_requested" };
    case "pending":
