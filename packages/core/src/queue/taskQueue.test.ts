...
describe("TaskQueue", () => {
  it("rejects duplicate task ids", () => {
    const queue = new TaskQueue({ retryPolicy: { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 1000 } });
    queue.enqueue(task());
  ...
  it("claims only eligible pending tasks", () => {
    const queue = new TaskQueue({ retryPolicy: { maxAttempts: 3, baseDelayMs: 100, maxDelayMs: 1000 } });
    queue.enqueue(task());
    queue.enqueue(task({ id: "future", idempotencyKey: "idem-2", availableAt: "2026-09-25T00:00:00.000Z", priority: 99 }));
