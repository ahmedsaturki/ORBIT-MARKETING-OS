...
if (parsed.protocol !== "https:")
  throw new Error("Live ORBIT URL must use HTTPS");
...
for (const fragment of ["ORBIT Marketing OS", "مركز تشغيل تسويقك"]) {
...
if (body.includes("Orbit Marketing OS - منصة تشغيل وتسويق وأتمتة شاملة")) {
  throw new Error(
    "Live ORBIT page is still serving the legacy main/Vite surface",
  );
}
...
for (const pathname of [
  ...
  if (!response.ok) {
    throw new Error(
      `Live ORBIT route ${pathname} returned HTTP ${response.status}`,
    );
  }
}
...
if (!manifest.ok)
  throw new Error("Live ORBIT manifest returned HTTP " + manifest.status);
const manifestBody = await manifest.json();
if (
  typeof manifestBody?.name !== "string" ||
  !/ORBIT/i.test(manifestBody.name)
) {
  throw new Error("Live ORBIT manifest branding mismatch");
}
if (manifestBody?.start_url !== "/")
  throw new Error("Live ORBIT manifest start_url mismatch");
if (manifestBody?.display !== "standalone")
  throw new Error("Live ORBIT manifest display mismatch");

const serviceWorker = await get("/sw.js", "text/javascript,*/*");
if (!serviceWorker.ok)
  throw new Error(
    "Live ORBIT service worker returned HTTP " + serviceWorker.status,
  );
const swBody = await serviceWorker.text();
if (!swBody.includes("addEventListener"))
  throw new Error("Live ORBIT service worker is invalid");
...
for (const [header, expected] of [
  ...
  if (value !== expected) {
    throw new Error(
      `Live ORBIT security header ${header} mismatch: ${value ?? "missing"}`,
    );
  }
}
...
if (missing.status !== 404) {
  throw new Error(
    "Live ORBIT unknown route must return HTTP 404, got " + missing.status,
  );
}
