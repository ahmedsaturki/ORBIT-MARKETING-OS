    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>
          الهاتف للمراقبة والتحكم الخفيف. الأسرار والتشغيل الثقيل يبقون في runtime المحلي على سطح المكتب.
        </Text>
        ...
        {health ? (
          <View style={styles.grid}>
            <View style={styles.card}><Text style={styles.label}>الخدمة</Text><Text style={styles.value}>{health.service}</Text></View>
            <View style={styles.card}><Text style={styles.label}>AI</Text><Text style={styles.value}>{health.provider ?? "غير متاح"}</Text></View>
            <View style={styles.card}><Text style={styles.label}>النموذج</Text><Text style={styles.value}>{health.model ?? "غير متاح"}</Text></View>
            <View style={styles.card}><Text style={styles.label}>الرؤية</Text><Text style={styles.value}>{health.visionConfigured ? "مضبوطة" : "غير مضبوطة"}</Text></View>
          </View>
        ) : null}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: "#0f172a", borderColor: "#1e293b", borderWidth: 1, borderRadius: 18, padding: 18, gap: 10 },
  grid: { gap: 12 },
  label: { color: "#64748b", fontSize: 12, fontWeight: "700" },
  value: { color: "#e2e8f0", fontSize: 16, fontWeight: "800" },
  input: { backgroundColor: "#020617", borderColor: "#334155", borderWidth: 1, borderRadius: 12, color: "#f8fafc", padding: 12 },
  button: { backgroundColor: "#10b981", borderRadius: 12, padding: 13, alignItems: "center" },
