...
test("home page exposes core product navigation", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: /مركز تشغيل تسويقك/ }),
  ).toBeVisible();
...
test("pricing page renders all license plans without fake checkout links", async ({
  page,
}) => {
  await page.goto("/pricing/");
  await expect(
    page.getByRole("heading", { name: "الخطط والأسعار" }),
  ).toBeVisible();
...
test("legal pages are reachable from navigation", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "شروط الاستخدام" }),
  ).toBeVisible();
  await page.goto("/legal/refunds/");
  await expect(
    page.getByRole("heading", { name: "سياسة الاسترداد" }),
  ).toBeVisible();
  await page.goto("/legal/eula/");
  await expect(
    page.getByRole("heading", { name: "اتفاقية ترخيص الاستخدام" }),
  ).toBeVisible();
});
...
test("production security headers are present on the static delivery path", async ({
  request,
}) => {
...
test("PWA manifest and service worker assets are reachable", async ({
  request,
}) => {
