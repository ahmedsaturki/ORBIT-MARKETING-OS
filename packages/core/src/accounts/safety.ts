...
export function calculateWarmupBudget(
  ...
  if (!Number.isInteger(daysActive) || daysActive < 0)
    throw new RangeError("daysActive must be non-negative");
  if (
    baseDailyLimit < 1 ||
    weeklyGrowth < 0 ||
    maxDailyLimit < baseDailyLimit
  ) {
  ...
  const dailyLimit = Math.max(
    baseDailyLimit,
    Math.min(maxDailyLimit, Math.floor(rawLimit)),
  );
...
export function calculateHealthScore(activity: AccountActivity): number {
  if (
    activity.tasksAttempted < 0 ||
    activity.tasksSucceeded < 0 ||
    activity.tasksFailed < 0 ||
    activity.challenges < 0
  ) {
    throw new RangeError("activity counts must be non-negative");
  }
  if (
    activity.tasksSucceeded + activity.tasksFailed >
    activity.tasksAttempted
  ) {
  ...
  const ageBonus = Math.min(
    10,
    Math.floor(Math.max(0, activity.daysActive) / 7),
  );
