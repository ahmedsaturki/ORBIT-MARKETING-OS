...
test.describe("Tauri renderer capability isolation (SEC-03)", () => {
  ...
  test("native queue recovery returns interrupted sync work to pending", async () => {
    ...
    , `e2e-recovery-account-${suffix}`)) as {
      id: string;
      status: string;
    ...
    const claimed = (await page!.evaluate(async (timestamp) =>
      window.__TAURI_INTERNALS__.invoke("task_claim_next", { now: timestamp }),
    , now)) as { id: string; status: string };
